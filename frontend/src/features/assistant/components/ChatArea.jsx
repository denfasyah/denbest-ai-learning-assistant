    ...
    <div className="flex flex-1 flex-col h-full overflow-hidden bg-[#020204]">
      ...
      >
        {messages.map((message, index) => (
          <div
            key={index}
            className={`flex items-start gap-5 ${message.role === "user" ? "flex-row-reverse" : "flex-row"} animate-in fade-in slide-in-from-bottom-4 duration-500`}
          >
        ...
        <div className="max-w-4xl mx-auto">
           <div className="relative flex items-end gap-3 rounded-[2.5rem] border border-white/[0.05] bg-white/[0.01] p-3 focus-within:border-indigo-500/30 transition-all shadow-2xl">
             ...
             <textarea
               ...
               className="max-h-40 min-h-12 flex-1 resize-none bg-transparent px-2 py-3 text-sm text-white outline-none placeholder:text-slate-700 font-medium"
             />

             <div className="flex items-center gap-2">
               <button className="h-12 w-12 rounded-full hidden sm:flex items-center justify-center text-slate-600 hover:text-indigo-400 hover:bg-white/5 transition-all shrink-0">
           ...
           <p className="mt-4 text-center text-[9px] text-slate-700 font-black uppercase tracking-[0.2em]">
